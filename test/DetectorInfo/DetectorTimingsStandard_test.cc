...
unsigned int testTriggerTime(detinfo::DetectorTimings const& timings) {
  ...
  static_assert(util::is_same_decay_v
    <decltype(time.quantity()), util::quantities::microsecond>);
...
unsigned int testSimulationTimes(detinfo::DetectorTimings const& timings) {
  ...
  static_assert(util::is_same_decay_v
    <decltype(startTime.quantity()), util::quantities::microsecond>);
  ...
  auto const elecStartTime
    = timings.startTime<electronics_time, simulation_time>();
  static_assert(util::is_same_decay_v
    <decltype(elecStartTime.quantity()), util::quantities::nanosecond>);
  
  if (elecStartTime == -expectedStartTime) {
    mf::LogVerbatim("DetectorTimingsStandard_test")
      << "DetectorTimings::startTime<electronics_time, simulation_time>() => "
      << elecStartTime;
  }
  else {
    ++nErrors;
  ...
  static_assert(util::is_same_decay_v
    <decltype(inputTime.quantity()), util::quantities::nanosecond>);
  ...
  if (time.value() == expectedTime) {
    mf::LogVerbatim("DetectorTimingsStandard_test")
      << "DetectorTimings::toElectronicsTime<simulation_time>("
      << inputTime << ") => " << time;
  }
  else {
    ++nErrors;
...
unsigned int testTriggerTimes(detinfo::DetectorTimings const& timings) {
  ...
  auto const elecStartTime
    = timings.startTime<electronics_time, trigger_time>();
  static_assert(util::is_same_decay_v
    <decltype(elecStartTime.quantity()), util::quantities::microsecond>);
  
  if (elecStartTime.value() == -expectedStartTime) {
    mf::LogVerbatim("DetectorTimingsStandard_test")
      << "DetectorTimings::startTime<electronics_time, trigger_time>() => "
      << elecStartTime;
  }
  else {
    ++nErrors;
  ...
  static_assert(util::is_same_decay_v
    <decltype(inputTime.quantity()), util::quantities::microsecond>);
  ...
  static_assert(util::is_same_decay_v
    <decltype(trigTime.quantity()), util::quantities::microsecond>);
...
unsigned int testOpticalClockTimings(detinfo::DetectorTimings const& timings) {
  ...
  // frequency
  double const expectedFrequency
    = timings.detClocks().OpticalClock().Frequency(); // MHz
  
  auto const frequency = timings.OpticalClockFrequency();
  static_assert
    (util::is_same_decay_v<decltype(frequency), util::quantities::megahertz>);
  ...
  if (frequency.value() == expectedFrequency) {
    ...
    mf::LogProblem("DetectorTimingsStandard_test")
      ...
      << " MHz, but got " << frequency << " instead";
  }
  
  // period
  double const expectedPeriod
    = timings.detClocks().OpticalClock().TickPeriod(); // MHz
  
  auto const period = timings.OpticalClockPeriod();
  static_assert(
    util::is_same_decay_v
      <decltype(period)::quantity_t, util::quantities::microsecond>
    );
  ...
  // in trigger scale
  auto const expectedTrigStartTime
    = expectedStartTime - timings.detClocksUnits().TriggerTime();
  
  auto const trigStartTime
    = timings.startTime<optical_time, trigger_time>();
  static_assert(util::is_same_decay_v
    <decltype(trigStartTime.quantity()), util::quantities::microsecond>);
  
  if (trigStartTime == expectedTrigStartTime) {
    mf::LogVerbatim("DetectorTimingsStandard_test")
      << "DetectorTimings::startTime<optical_time, trigger_time>() => "
      << trigStartTime;
  }
  else {
    ++nErrors;
  ...
  //
  double const expectedTime_us
    = inputTick_count * timings.detClocks().OpticalClock().TickPeriod();
  
  optical_tick_d inputTick_d { inputTick_count };
  static_assert(util::is_same_decay_v
    <decltype(inputTick_d.quantity()), util::quantities::tick_d>);
  ...
  if (time.value() == expectedTime_us) {
    mf::LogVerbatim("DetectorTimingsStandard_test")
      << "DetectorTimings::toElectronicsTime<optical_tick>(" << inputTick_d
      << ") => " << time;
  }
  else {
    ++nErrors;
  ...
  static_assert(util::is_same_decay_v
    <decltype(tick.quantity()), util::quantities::tick_d>);
  
  if (tick == inputTick_d) {
    mf::LogVerbatim("DetectorTimingsStandard_test")
      << "DetectorTimings::toTick<optical_tick, "
      << detinfo::timescales::timescale_traits<decltype(time)::category_t>
        ::name()
      << ">(" << time << ") => " << tick;
  }
  else {
    ...
    mf::LogProblem("DetectorTimingsStandard_test")
      ...
      << ", but got " << tick << " instead";
  }
  
  // to trigger time (truncated tick):
  
  double const expectedTrigTime_us
    = expectedTime_us - timings.detClocks().TriggerTime();
  double const expectedTrigTime_truncated_us
    = static_cast<int>(inputTick_count)
      * timings.detClocks().OpticalClock().TickPeriod()
      - timings.detClocks().TriggerTime();
  
  auto const inputTick = optical_tick::castFrom(inputTick_count);
  static_assert(util::is_same_decay_v
    <decltype(inputTick.quantity()), util::quantities::tick>);
  ...
  static_assert(util::is_same_decay_v
    <decltype(trigTick.quantity()), util::quantities::tick>);
  
  if (trigTick == inputTick) {
    mf::LogVerbatim("DetectorTimingsStandard_test")
      << "DetectorTimings::toTick<optical_tick, trigger_time>("
      << expectedTrigTime_us << ") => " << trigTick;
  }
  else {
    ++nErrors;
    mf::LogProblem("DetectorTimingsStandard_test")
      << "Trigger time " << expectedTrigTime_us
      << " us is expected to be optical tick " << inputTick
