 ...
 * @brief  Unit test for `lardataalg/Utilities/quantities/frequency.h` header.
...
void test_frequency_literals()
{
  ...
  static_assert(std::is_same<decltype(f_kHz), util::quantities::kilohertz const>());
  static_assert(double(f_kHz) == 20.0);
  static_assert(f_kHz == 20.0_kHz);

  constexpr auto f_MHz = 200_MHz;
  static_assert(std::is_same<decltype(f_MHz), util::quantities::megahertz const>());
  static_assert(double(f_MHz) == 200.0);
  static_assert(f_MHz == 200.0_MHz);

  constexpr auto f_GHz = 5_GHz;
  static_assert(std::is_same<decltype(f_GHz), util::quantities::gigahertz const>());
...
void test_frequency_special_operations()
{
  ...
  static_assert(std::is_same<decltype(10 / 5_us), util::quantities::megahertz>());
  static_assert(10 / 5_us == 2_MHz);
  static_assert(10'000.0 / 5_ms == 2_MHz);

  // ---------------------------------------------------------------------------

  static_assert(std::is_same<decltype(10 / 2_MHz), util::quantities::microsecond>());
