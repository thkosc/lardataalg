  ...
  public:
    ...
    struct Configuration_t {
      ...
      fhicl::Atom<double> SternheimerX1{
        Name("SternheimerX1"),
        Comment("parameter x_1 of Sternheimer correction delta = 2log(10) x - cbar + { a (x_1-x)^k "
                "} theta(x1-x), x = log10(p/m)")};
      ...
      fhicl::Atom<double> DriftVelFudgeFactor{
        ...
        Comment("Allows a scaling factor to fudge the drift velocity calculation (as suggested by "
                "DriftVel Stancari")};
    ...
    void
    SetGeometry(const geo::GeometryCore* g)
    {
      fGeo = g;
    }
    void
    SetLArProperties(const detinfo::LArProperties* lp)
    {
      fLP = lp;
    }
    void
    SetDetectorClocks(const detinfo::DetectorClocks* clks)
    {
      fClocks = clks;
    }
