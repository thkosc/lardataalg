...
std::string sim::TruthInteractionTypeName(int type)
{
  ...
  case simb::kResCCNuBarNeutronRhoMinus:
    return "resonant charged current antineutrino neutron rho-";
  ...
  case simb::kResCCNuBarSigmaMinusKaon0:
    return "resonant charged current antineutrino Sigma- kaon0";
  ...
  case simb::kResCCNuBarNeutronPiPlusPiMinus:
    return "resonant charged current antineutrino neutron pi+ pi-";
  case simb::kResCCNuBarNeutronPi0Pi0:
    return "resonant charged current antineutrino neutron pi0 pi0";
                                  ...
                                  RescatterCategory cat /* = RescatterCategory::LArSoftDefault */)
{
  ...
  switch (cat) {
  case RescatterCategory::GENIE_INukeFateHA: return GENIE_INukeFateHA_RescatteringName(code);
  default:
    return "unknown category "s + std::to_string(static_cast<int>(cat)) + " (code: "s +
           std::to_string(code) + ")"s;
  } // switch(code)
...
std::string sim::GENIE_INukeFateHA_RescatteringName(int code)
{
  ...
  return genie::INukeHadroFates::AsString(static_cast<genie::INukeFateHA_t>(code));
  ...
  default: return "unknown ("s + std::to_string(code) + ")"s;
  } // switch(code)
